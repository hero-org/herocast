...
- ~~**Tauri desktop consumer** — the desktop build currently targets the Next app; how it consumes the TanStack app must be resolved **before Phase 4 cutover** (not Phase 1/2).~~ **LEADING CANDIDATE (still deferred past #13):** wrap the same TanStack Start app in the **Deno desktop runtime** (auto-detects TanStack Start; reuses the host-portability seam — seam fall-through spike-verified). **Spike caveat:** it's Chromium/CEF-backed — Electron-class (~295 MB macOS hello-world), *not* small like Tauri — so it's a DX-vs-size call, and canary/experimental today. If a small native binary is required, Tauri stays the alternative. Full analysis + the local-vs-remote-server (secrets) decision live in `desktop-deno-runtime.md`. Lands after cutover.
