...
- ~~**Tauri desktop consumer** — the desktop build currently targets the Next app; how it consumes the TanStack app must be resolved **before Phase 4 cutover** (not Phase 1/2).~~ **LEADING CANDIDATE (still deferred past #13):** wrap the same TanStack Start app in the **Deno desktop runtime** (auto-detects TanStack Start; reuses the host-portability seam — seam fall-through spike-verified; built+launched+rendered on macOS). **Key edge:** one codebase, `--backend`-selectable size — `cef` (Chromium, ~295 MB macOS measured, consistent rendering + DevTools) or native `webview` (Tauri-like size, **untested + per-OS rendering risk**); pin `desktop.backend` (canary defaulted to CEF despite docs). Canary/experimental today. Full analysis + the local-vs-remote-server (secrets) decision + open probes live in `desktop-deno-runtime.md`. Lands after cutover.
