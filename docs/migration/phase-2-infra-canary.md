...
**Coexistence guarantee:** this unit touches `.conductor/settings.toml`, `.github/workflows/cf-web-canary.yml`, `wrangler.jsonc` (the **canary's own** config — enables the `cf.herocast.xyz` custom-domain route), `docs/migration/*`, and `strategy.md`'s status cell. Zero changes to any **live-app** file: `app/`, `pages/`, `next.config.mjs`, `vercel.json`, `src/globals.css`, `package.json`, `vite.config.mts`. The only DNS provisioned is the new `cf.herocast.xyz` subdomain; the Vercel apex/`www` records stay DNS-only, so the live app is untouched (verified still 200 via Vercel).
...
> **STATUS — ✅ done & verified live at https://cf.herocast.xyz.** Secrets set; the `herocast.xyz` zone was moved to Cloudflare (NS now `damon/ximena.ns.cloudflare.com`; Vercel apex/`www` kept DNS-only so the live app is unaffected); the custom-domain route is enabled and the CI blocking smoke is green. Adding the route disabled the `*.workers.dev` URL, which is expected — `cf.herocast.xyz` is the canonical canary URL. The Path A/B steps below are kept as the record of how it was set up (Path B was completed early). One CI gotcha hit along the way: a wrong/under-scoped token, then a fixed `CLOUDFLARE_ACCOUNT_ID`, then a post-deploy propagation race — all resolved (smoke retries 8×).

Originally: `herocast.xyz` DNS was on **Namecheap** (`dns{1,2}.registrar-servers.com` → Vercel), **not Cloudflare**. A Workers `custom_domain` requires the zone active on Cloudflare, so the canary first shipped on its `*.workers.dev` URL (Path A) and `cf.herocast.xyz` followed once the zone moved (Path B).
