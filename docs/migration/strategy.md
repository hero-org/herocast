...
- **Before #13 cutover:** full parity + duplication + dead-code sweep; the desktop-consumer question (`phase-1.md §10`) has a leading candidate — **Deno desktop runtime** wrapping the TanStack app (`desktop-deno-runtime.md`; spike-verified, but it's Chromium/Electron-class — *not* small like Tauri, so it's a DX-vs-size call) — but ships *after* cutover, so it is not a blocker for the sweep.
...
- **[post-#13] Desktop consumer — leading candidate: TanStack app via Deno desktop runtime.** Closes the `phase-1.md §10` open question. Reuses the host-portability seam (a Deno build target alongside CF/Vercel; seam fall-through spike-verified). Default to the **hybrid** shape (native UI, data/auth server fns proxy the remote worker so secrets stay server-side). **Spike caveat:** Deno desktop is Chromium/CEF-backed — **Electron-class, ~295 MB on macOS for hello-world, not small like Tauri** — so this is a DX-vs-binary-size decision, and it's canary/experimental today. Full analysis + spike scaffold (`scripts/spikes/deno-desktop/`) in `desktop-deno-runtime.md`. Deferred until after cutover.
