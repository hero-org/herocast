...
- **Before #13 cutover:** full parity + duplication + dead-code sweep; the desktop-consumer question (`phase-1.md §10`) has a leading candidate — **Deno desktop runtime** wrapping the TanStack app (`desktop-deno-runtime.md`; spike-verified end-to-end on macOS, size is `--backend`-selectable: CEF=295 MB measured, native `webview`=Tauri-like but untested) — but ships *after* cutover, so it is not a blocker for the sweep.
...
- **[post-#13] Desktop consumer — leading candidate: TanStack app via Deno desktop runtime.** Closes the `phase-1.md §10` open question. Reuses the host-portability seam (a Deno build target alongside CF/Vercel; seam fall-through spike-verified; built+launched+rendered on macOS). Default to the **hybrid** shape (native UI, data/auth server fns proxy the remote worker so secrets stay server-side). **Key edge:** one codebase, `--backend`-selectable size — `cef` (Chromium, 295 MB macOS measured, consistent rendering + DevTools) or `webview` (native OS webview, Tauri-like size, **untested + per-OS rendering risk**). Pin `desktop.backend` (canary defaulted to CEF despite docs). Canary/experimental today. Open probe: `--backend webview` against herocast's real UI. Full analysis + spike (`scripts/spikes/deno-desktop/`) in `desktop-deno-runtime.md`. Deferred until after cutover.
