...
/**
 * CompactCastRow — dense two-line list-pane row for the split-pane /feeds layout.
 *
 * Visual hierarchy (top to bottom):
 *   [Avatar 32px]  Author Name  @handle              ← line 1: identity (muted)
 ...
 * - No engagement counts / reaction icons.
 * - No `read more` affordance — list rows are always single-line truncated.
 ...
 *   Tailwind `line-clamp-1` instead. Mentions / links stay non-interactive in
...
export interface CompactCastRowProps {
  ...
  showChannel?: boolean;
  /** Fires on row click; parent gates desktop preview vs mobile thread-view. */
  onSelect?: () => void;
}

const CompactCastRowComponent: React.FC<CompactCastRowProps> = ({ cast, isSelected, showChannel, onSelect }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col min-w-0 flex-1 gap-y-0.5">
        ...
        <div className="flex items-center gap-x-1.5 text-sm leading-5 text-foreground/60 min-w-0">
          <span className="font-medium text-foreground/80 truncate">{displayName}</span>
          {username && <span className="truncate">@{username}</span>}
        </div>

        {/* Line 2 — cast content (loudest element) */}
        <div className="text-md leading-5 text-foreground line-clamp-1 break-words" title={cast.text}>
          {cast.text}
        </div>

        {/* Meta row — muted timestamp · optional channel */}
        <div className="flex items-center gap-x-1.5 text-xs leading-4 text-foreground/60 min-w-0">
...
export const CompactCastRow = React.memo(CompactCastRowComponent, (prev, next) => {
  ...
  // visually load-bearing fields change. Reactions / embeds are irrelevant here.
  return (
