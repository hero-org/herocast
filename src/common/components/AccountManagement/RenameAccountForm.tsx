...
const RenameAccountForm = ({
    ...
    useEffect(() => {
        const getUserInProtocol = async () => {
            ...
            const user = (
                await neynarClient.fetchBulkUsers([Number(account.platformAccountId!)], { viewerFid: APP_FID })
            )?.users?.[0];
            if (user) {
            ...
            <form onSubmit={form.handleSubmit(renameAccount)} className="space-y-4">
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            ...
                            <FormControl>
                                <Input
                                    placeholder={hasUsernameInProtocol ? userInProtocol?.username : "New username"}
                                    {...field}
                                />
                            </FormControl>
                ...
                {hasUsernameInProtocol && (
                    <p className="text-sm text-muted-foreground">
                        Renaming requires two signatures: one to unregister the old username and one to register the new
                        username.
                    </p>
                )}
