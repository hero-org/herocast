...
describe("getTextMatchCondition", () => {
  ...
  test("should handle single word", () => {
    expect(getTextMatchCondition("optimistic")).toBe(
  ...
  test("should handle phrase without quotes", () => {
    expect(getTextMatchCondition("looking at clouds")).toBe(
  ...
  test("should handle combination of quotes and boolean operator", () => {
    ...
    expect(result).toBe(
      `${TEXT_COLUMN} ~* '\\moptimistic\\M' AND tsv @@ websearch_to_tsquery('english', '-potato')`,
    );
  });
  ...
  test("should handle complex query with AND operator", () => {
    ...
    expect(result).toBe(
      `${TEXT_COLUMN} ~* '\\mhappy days\\M' AND tsv @@ websearch_to_tsquery('english', 'sunshine')`,
    );
  });
