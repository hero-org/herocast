...
export const callCreateSignerRequest = async (
  data: CreateSignerRequestType,
): Promise<WarpcastLoginType> => {
...
const callGetSignerRequestStatus = async (signerToken: string) => {
  ...
  const response = await fetch(
    `/api/signerRequest?signerToken=${signerToken}`,
    {
      method: "GET",
      headers: {
...
export const getSignerRequestStatus = async (signerToken: string) => {
  const data = await (
    await fetch(
      `${WARPCAST_API_ENDPOINT}signed-key-request?token=${signerToken}`,
      { headers },
    )
  ).json();
...
const getWarpcastSignerStatus = async (
  signerToken: string,
): Promise<{ status: WarpcastLoginStatus; data: any }> => {
