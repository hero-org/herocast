...
export function channelPlugin({ scanner, parser }) {
  const { SLASH, UNDERSCORE, HYPHEN, SPACE } = scanner.tokens;
  const { alpha, numeric, alphanumeric, emoji, symbol } = scanner.tokens.groups;

  const WhiteSpace = parser.start.tt(SPACE);
