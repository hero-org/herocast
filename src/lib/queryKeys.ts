...
export const queryKeys = {
  ...
  feeds: {
    all: ['feeds'] as const,
    trending: (options?: { cursor?: string; limit?: number }) =>
      ['feeds', 'trending', options ?? {}] as const,
  ...
  profiles: {
    ...
    bulk: (fids: number[]) =>
      ['profiles', 'bulk', fids.slice().sort((a, b) => a - b).join(',')] as const,
  ...
  casts: {
    all: ['casts'] as const,
    byHash: (hash: string, viewerFid?: number) =>
      ['casts', 'byHash', hash, viewerFid ?? null] as const,
    thread: (hash: string, viewerFid?: number) =>
      ['casts', 'thread', hash, viewerFid ?? null] as const,
  ...
  search: {
    all: ['search'] as const,
    casts: (term: string, filters?: Record<string, unknown>) =>
      ['search', 'casts', term, filters ?? {}] as const,
...
export type FeedQueryKey = ReturnType<typeof queryKeys.feeds.trending | typeof queryKeys.feeds.following | typeof queryKeys.feeds.channel>;
