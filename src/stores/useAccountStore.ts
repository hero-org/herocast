...
interface AccountStoreActions {
  ...
  updateAccountProperty: (accountId: string, property: keyof AccountObjectType, value: any) => void;
  loadFarcasterApiKey: (accountId: string) => Promise<void>;
...
const store = (set: StoreSet) => ({
    ...
    if (localOnly) {
      ...
      await getSupabaseClient()
        ...
        .then(({ error, data }) => {
          ...
          set((state) => {
            state.accounts.push({ ...account, channels: [], id: data[0].id });
          });
        });
    }
    ...
    set(async (state) => {
      await getSupabaseClient()
        ...
        .then(({ error, data }) => {
          ...
          const newChannel = { ...data[0], id: data[0].id } as ChannelType;
  ...
  loadFarcasterApiKey: async (accountId: string) => {
...
const fetchAllChannels = async (): Promise<ChannelType[]> => {
  ...
  do {
    ...
    const typedData = (data || []).map((d) => ({ ...d, id: d.id })) as ChannelType[];
