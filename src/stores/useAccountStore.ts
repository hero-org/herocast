...
interface AccountStoreActions {
  ...
  updatedPinnedChannelIndices: ({
    oldIndex,
    newIndex,
  }: UpdatedPinnedChannelIndicesProps) => void;
  setAccountActive: (
    accountId: UUID,
    name: string,
...
const store = (set: StoreSet) => ({
  ...
  addAccount: async (props: AddAccountProps) => {
    ...
    if (localOnly) {
      set((state) => {
        const channels = state.allChannels.filter((channel) =>
          includes(DEFAULT_LOCAL_ACCOUNT_CHANNELS, channel.name),
        );
  ...
  setSelectedChannelByName: (name: string) => {
    set((state) => {
      ...
      const channel = state.allChannels.find(
        (channel) =>
          channel.name === name ||
          channel.url === `https://warpcast.com/~/channel/${name}`,
      );
      if (channel) {
  ...
  updatedPinnedChannelIndices: async ({
    oldIndex,
    newIndex,
  }: UpdatedPinnedChannelIndicesProps) => {
    set((state) => {
      const account = state.accounts[state.selectedAccountIdx];
...
export const hydrateAccounts = async (): Promise<AccountObjectType[]> => {
  ...
  if (accountData.length === 0) {
    ...
    accounts = accountData.map((account) => {
      const channels: AccountChannelType[] = sortBy(
        account.accounts_to_channel,
        "index",
      ).map((accountToChannel) => ({
    ...
    if (fids.length) {
      ...
      try {
        const users = (
          await neynarClient.fetchBulkUsers(fids, { viewerFid: APP_FID })
        ).users;
        accounts = accounts.map((account) => {
          const user = users.find(
            (user) => user.fid === Number(account.platformAccountId),
          );
          if (user) {
            account.user = user;
          }
  ...
  for (let i = 0; i < 9; i++) {
    accountCommands.push({
      ...
      enabled: () =>
        ...
        useAccountStore.getState().accounts[i].status ===
...
export const getChannelCommands = (state) => {
  let channelCommands: CommandType[] = [
    ...
    },
    {
      ...
      action: () => {
        ...
        const randomIndex = randomNumberBetween(
          0,
          state.allChannels.length - 1,
        );
    ...
    },
    {
      ...
      action: () => {
        ...
        const currentIdx = getCurrentChannelIndex(
        ...
        if (nextIdx === 1) {
          ...
          state.setSelectedChannelUrl(
            channels[nextIdx - CUSTOM_CHANNEL_COUNT].url,
          );
        }
      },
    },
    {
      ...
      action: () => {
        ...
        if (previousIdx === 1) {
          ...
          state.setSelectedChannelUrl(
            channels[previousIdx - CUSTOM_CHANNEL_COUNT].url,
          );
        }
      },
    },
  ...
  if (channels) {
    channelCommands = channelCommands.concat(
      getCommandsForPinnedChannels(channels, state),
    );
  }
