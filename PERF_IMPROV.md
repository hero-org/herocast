...
const getNeynarClient = () => {
  return import('@neynar/nodejs-sdk').then(
    module => new module.NeynarAPIClient(apiKey)
  );
};
```
...
export const usePerformanceTracker = () => {
  ...
  const endTiming = useCallback((name: string, threshold = 100) => {
    ...
    const metric: PerformanceMetric = {
      ...
      timestamp: Date.now(),
      threshold,
      status: duration < threshold ? 'good' : 
              duration < threshold * 2 ? 'warning' : 'critical'
    };
...
export const runPerformanceAudit = () => {
  ...
  if (violations.length > 0) {
    ...
    violations.forEach(({ name, actual, budget }) => {
      console.warn(`${name}: ${actual.toFixed(1)}ms (budget: ${budget}ms) - ${((actual - budget) / budget * 100).toFixed(1)}% over`);
    });
    console.groupEnd();
  } else {
