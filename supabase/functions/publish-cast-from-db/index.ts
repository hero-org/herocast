    ...
    castAddBody.embeds.forEach((embed) => {
      if ("castId" in embed) {
        embed.castId = {
          ...
          hash: new Uint8Array(
            embed?.castId?.hash.split(",").map((x: string) => parseInt(x)),
          ),
        };
      }
    });
...
const publishDraft = async (supabaseClient, draftId) => {
  return Sentry.withScope(async (scope) => {
    ...
    if (getDraftError || drafts?.length !== 1) {
      const errorMessage =
        getDraftError || `no draft returned for id ${draftId}`;
    ...
    if (getAccountError || accounts?.length !== 1) {
      console.error(
        getAccountError || `no account returned for id ${draft.account_id}`,
      );
      return;
    }
    ...
    if (updateDraftStatusError) {
      console.error(
        `Failed to update draft status to publishing for id ${draftId}: ${updateDraftStatusError}`,
      );
      return;
    }
...
Deno.serve(async (req) => {
  return Sentry.withScope(async (scope) => {
    ...
    try {
      ...
      const next5Minutes = new Date(
        now.getTime() + 5 * 60000 - 1,
      ).toISOString();
      ...
      if (!drafts || drafts?.length === 0) {
        console.log(
          `No drafts to publish between: ${invocationTime} and ${next5Minutes}`,
        );
        return new Response("ok", {
          headers: { "Content-Type": "application/json" },
          status: 200,
      ...
      console.log(
        `Drafts to publish between: ${invocationTime} and ${next5Minutes}:`,
        drafts.length,
        error,
      );
